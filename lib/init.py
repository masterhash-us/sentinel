...
def has_saros_conf():
    ...
    valid_saros_conf = False

    # ensure saros_conf exists & readable
    #
    # if not, print a message stating that Saros Core must be installed and
    # configured, including JSONRPC access in saros.conf
    try:
        f = io.open(config.saros_conf)
        valid_saros_conf = True
    except IOError as e:
        print(e)

    return valid_saros_conf
...
def main():
    ...
    if not has_saros_conf():
        print("SarosCore must be installed and configured, including JSONRPC access in saros.conf")
        sys.exit(1)
