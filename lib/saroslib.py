...
def is_valid_saros_address(address, network='mainnet'):
    ...
    saros_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Saros address
        return False

    if (address_version != saros_version):
        return False
...
# shims 'til we can fix the sarosd side
def SHIM_serialise_for_sarosd(sentinel_hex):
    from models import SAROSD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = SAROSD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in sarosd
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # sarosd expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    sarosd_hex = serialise(obj)
    return sarosd_hex


# shims 'til we can fix the sarosd side
def SHIM_deserialise_from_sarosd(sarosd_hex):
    from models import SAROSD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(sarosd_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('saros.conf')
