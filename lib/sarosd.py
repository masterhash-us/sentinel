...
class SarosDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some sarosd methods
    ...
    @classmethod
    def from_saros_conf(self, saros_dot_conf):
        from saros_config import SarosConfig
        config_text = SarosConfig.slurp_config_file(saros_dot_conf)
        creds = SarosConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = saroslib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = saroslib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = saroslib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_sarosd_proto_version_with_sentinel_ping)
