...
class GovernanceObject(BaseModel):
    ...
    # sync sarosd gobject list with our local relational DB backend
    @classmethod
    def sync(self, sarosd):
        golist = sarosd.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_sarosd(sarosd, item)
    ...
    @classmethod
    def import_gobject_from_sarosd(self, sarosd, rec):
        import decimal
        import saroslib
        import inflection
        ...
        # shim/sarosd conversion
        object_hex = saroslib.SHIM_deserialise_from_sarosd(object_hex)
        objects = saroslib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from sarosd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from sarosd, with every run
        ...
        # get/create, then sync payment amounts, etc. from sarosd - Sarosd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(sarosd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from sarosd! %s" % e)
            govobj.vote_delete(sarosd)
            return (govobj, None)
    ...
    def vote_delete(self, sarosd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(sarosd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, sarosd, signal, outcome):
        import saroslib
        ...
        output = sarosd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = saroslib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(sarosd, signal)

    def sync_network_vote(self, sarosd, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = sarosd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = SAROSD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 saros addr, non-multisig
            if not saroslib.is_valid_saros_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Saros address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (saroslib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/SarosDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import saroslib
        obj_data = saroslib.SHIM_serialise_for_sarosd(self.serialise())
    ...
    def prepare(self, sarosd):
        try:
            object_hash = sarosd.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = SAROSD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not saroslib.is_valid_saros_address(addr, config.network):
    ...
    def hash(self):
        import saroslib
        return saroslib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = SAROSD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, sarosd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - sarosd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, sarosd):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - sarosd.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, sarosd):
        now = int(time.time())
        return (self.created_at < (now - sarosd.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, sarosd):
        if self.is_expired(sarosd):
            return False

        return True

    def is_deletable(self, sarosd):
        if self.is_expired(sarosd):
            return True
